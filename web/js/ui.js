...
(function () {
  ...
  function mesNome(ym) {
    ...
    // O ano só aparece quando não é o corrente: "Setembro" e "Setembro de
    // 2027" na mesma régua não podem se chamar igual.
    const anoAtual = +hoje().slice(0, 4);
    return y === anoAtual ? nome : `${nome} de ${y}`;
  }
  const diaCurto = (d) => (d ? `${String(d).slice(8, 10)}/${String(d).slice(5, 7)}` : "");
  // Número curto para o gráfico. Uma casa decimal SEMPRE que ela existe:
  // arredondar 29.500 para "30k" ao lado de uma tela que diz R$ 29.500,00 é
  // o tipo de diferença que faz duvidar do app inteiro.
  function curto(v) {
    if (ocultar) return "•••";
    const n = Math.abs(+v || 0);
    if (n < 1000) return String(Math.round(n));
    const k = n / 1000;
    return k.toLocaleString(cfg.LOCALE || "pt-BR",
      { minimumFractionDigits: 0, maximumFractionDigits: 1 }) + "k";
  }
  ...
  // Janela da régua: um ano para trás e um ano para a frente do mês corrente.
  // A conta antiga cortava os 24 mais NOVOS, então uma compra parcelada em
  // muitas vezes (que cria um mês lá na frente para cada parcela) empurrava
  // a janela e o mês corrente saía da régua. Sumir da régua parece sumir do
  // app, com os lançamentos dentro.
  const HORIZONTE = 12;
  function mesesDisponiveis() {
    const atual = hoje().slice(0, 7);
    const limite = Bl.ymAdd(atual, HORIZONTE);
    const inicio = Bl.ymAdd(atual, -HORIZONTE);
    ...
    // Do mais antigo para o mais novo, como o tempo anda, e sempre dentro da
    // janela: o mês corrente jamais fica de fora.
    return Array.from(set).filter((m) => m >= inicio && m <= limite).sort();
  }
